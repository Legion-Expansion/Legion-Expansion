...
for unit_path in mla_units["units"]:
    ...
    if "recon" in unit and "observer" in unit["recon"] and "items" in unit["recon"]["observer"]:
    ...
    if old_buildable_types and not old_buildable_types.endswith(" - Custom1 - Custom2 - Custom3 - Custom4"):
        unit["buildable_types"] = "(" + old_buildable_types + ") - Custom1 - Custom2 - Custom3 - Custom4"

# Anti AA missile
load("/pa/units/air/support_platform/support_platform_tool_interception.json")["anti_entity_targets"] += [
...
# Anti Nuke missile
load("/pa/units/land/anti_nuke_launcher/anti_nuke_launcher_tool_weapon.json")["anti_entity_targets"] += [
    "/pa/units/land/l_nuke_launcher/l_nuke_launcher_ammo.json"
]

# Anti Tac Missiles
load("/pa/units/land/bot_sniper/bot_sniper_beam_tool_weapon.json")["anti_entity_targets"] += [
    "/pa/units/land/l_hover_tank_adv/l_hover_tank_adv_ammo.json"
]
...
for tool in map(load, anti_drop_tools):
    tool["anti_entity_targets"] += ["/pa/units/orbital/l_orbital_dropper/l_orbital_dropper_ammo.json"]
...
#########################################################################################
legion_shield = spec.parse_spec(loader, "/pa/units/land/l_shield_gen/anti_entity_targets.json")
for target in legion_shield["anti_entity_targets"]:
    ...
    if not is_legion:
        ...
        with open('client' + dst_trail_file, 'w', encoding='utf-8') as trail_file:
            pajson.dump_effect(spec.load_spec(loader, src_trail_file), trail_file, indent=2)


# Write out all changes to the mod server directory
for file_path, unit in file_cache.items():
    # If we have mode no changes, ignore them
